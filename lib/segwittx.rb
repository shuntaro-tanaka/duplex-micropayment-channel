...
class Segwittx
  ...
  def initialize(data=nil)
    @ver, @marker, @flag, @witness, @lock_time, @in, @out, @scripts = 1, 0, 1, [], 0, [], [], []
  ...
  # add an witness
  def add_witness(witness); (@witness ||= []) << witness; end
  ...
  def parse_data_from_io(data)
    ...
    @marker = buf.read(1).unpack("c")[0]

    @flag = buf.read(1).unpack("c")[0]
    ...
    witness_size = Bitcoin::Protocol.unpack_var_int_from_io(buf)
    @witness = []
    witness_size.times{
      length = Bitcoin::Protocol.unpack_var_int_from_io(buf)
      @witness << buf.read(length).unpack("H*")[0]
    }
  ...
  def wtxid
    ...
    @witness.each{|w|
      value = [w].pack("H*")
      pwitness << Bitcoin::Protocol.pack_var_int(value.size)
      pwitness << value
    }
    payload = [@ver].pack("V") << [0].pack("c") << [1].pack("c") << Bitcoin::Protocol.pack_var_int(@in.size) << pin <<
        Bitcoin::Protocol.pack_var_int(@out.size) << pout << Bitcoin::Protocol.pack_var_int(@witness.size) << pwitness << [@lock_time].pack("V")
    hash_from_payload(payload)
  end
